...
class LoginApp(ctk.CTk):
    ...
    def create_login_frame(self, login_type):
        ...
        self.logout_button = ctk.CTkButton(self.login_frame, text="Back", command=self.show_main_frame)
    ...
    def create_signup_frame(self):
        ...
        self.logout_button = ctk.CTkButton(self.signup_frame, text="Back", command=self.show_main_frame)
        self.logout_button.grid(row=9, column=1, pady=10)
    ...
    def show_success_page(self, role, username):
        ...
        self.logout_button = ctk.CTkButton(self.success_frame, text="Log Out", command=self.show_main_frame)
    ...
    def show_dashboard(self, role, username):
        ...
        self.label = ctk.CTkLabel(self.dashboard_frame, text=f"{role.capitalize()} Dashboard", font=ctk.CTkFont(size=20, weight="bold"))
        self.label.pack(pady=10)

        self.search_label = ctk.CTkLabel(self.dashboard_frame, text="Employees")
        self.search_label.pack(pady=5)

        self.tree = tk.Listbox(self.dashboard_frame)
        self.tree.pack(pady=10, fill="both", expand=True)

        self.load_users(username, role)
        
        self.logout_button = ctk.CTkButton(self.dashboard_frame, text="Log Out", command=self.show_main_frame)
        self.logout_button.pack(pady=10)
    
    def load_users(self, username, role):
        employees = self.db.get_employees(username, role)
        for employee in employees:
            self.tree.insert(tk.END, employee)
